...
import { CalendarIcon, X, CheckCircle2, Trash2 } from 'lucide-react';
...
import { taskSchema, TaskFormValues } from '../validators';
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="p-6 space-y-6">
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="space-y-2">
                  <FormLabel className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Task Title</FormLabel>
                  <FormControl>
                    <Input 
                      {...field} 
            ...
            <div className="grid grid-cols-2 gap-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="space-y-2">
                    <FormLabel className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Category</FormLabel>
                        ...
                        {categories.map((c) => (
                          <SelectItem key={c.id} value={c.id}>
                            <div className="flex items-center gap-2">
                              <div className={cn("size-2 rounded-full", getCategoryBg(c.color))} />
                              {c.name}
                            </div>
                          </SelectItem>
                        ))}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="space-y-2">
                    <FormLabel className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Due Date</FormLabel>
                    <Popover>
                      ...
                      <PopoverContent className="w-auto p-0 bg-[#16212e] border-[#223145]" align="start">
                        <Calendar
                          mode="single"
                          selected={new Date(field.value)}
                          onSelect={(date) => field.onChange(date ? format(date, 'MMM dd, yyyy') : '')}
                          initialFocus
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="space-y-2">
                  <FormLabel className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Description</FormLabel>
                  <FormControl>
                    <Textarea 
                      {...field} 
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="space-y-2">
                  <FormLabel className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Priority</FormLabel>
                  <FormControl>
            ...
            <div className="pt-4 flex items-center justify-between border-t border-[#223145] mt-6 -mx-6 px-6">
              {task ? (
                <div className="flex flex-col">
                  <span className="text-[10px] font-bold uppercase text-slate-500">Last Updated</span>
